...
export default function Workout({ index, id, update, remove }: { index: number, id: string, update: (id: string, name: string, value: any) => void, remove: () => void }) {
  ...
  return (
    <div className="flex justify-between gap-0.5 p-2 text-white bg-zinc-950 rounded-lg">
      {/* name */}
      ...
      <div className="flex gap-1.5">
        ...
        <Input className="w-fit text-white valid:border-emerald-500" placeholder="weight in kg" type="number" name="weight" onChange={changeHandler} min={0} max={900} pattern="^[0-9]{1,3}$" required />
      </div>
      <Button style={{
